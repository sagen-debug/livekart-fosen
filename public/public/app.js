(() => {
  ...
  function makeIcon(publicCode, delayMin) {
    ...
    const isBoat = ["800","805","810","830","835","850","880"].includes(String(code));

    const html = `
      <div class="vehicle-marker ${isBoat ? "vehicle-marker--boat" : ""}">
        <div>${escapeHtml(String(code))}</div>
  ...
  async function fetchVehicles() {
    ...
    // Skjul gamle kjøretøy i frontend
    const visible = [];
    let hiddenOld = 0;
    ...
    for (const v of visible) {
      ...
      const popupHtml = `
        <div style="font-family: system-ui, -apple-system, Segoe UI, Roboto, Arial, sans-serif;">
          <div style="font-weight: 800; margin-bottom: 6px;">Rute ${escapeHtml(String(code))}</div>
          <div><strong>Til:</strong> ${escapeHtml(String(dest))}</div>
    ...
    const hiddenStr = hiddenOld ? ` • Skjult (gammel pos.): ${hiddenOld}` : "";
  ...
  async function tick() {
    try {
      ...
      setStatus({
        ...
        meta: String(err && err.message ? err.message : err).slice(0, 140)
      });
    }
  }
