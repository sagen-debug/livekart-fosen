(() => {
  ...
  // Hvor lenge vi tolererer “gammel posisjon” før vi skjuler
  const HIDE_OLD_BUS_MS = 30 * 60 * 1000;   // 30 min
  const HIDE_OLD_BOAT_MS = 15 * 60 * 1000;  // 15 min

  // Ekstra “ghost-cap” i frontend for rute 870 (vis maks 2 aktive)
  const GHOST_CAP_CODES = new Set(["870"]);
  const MAX_ACTIVE_BOATS_PER_ROUTE = 2;
  const FERRY_CODES = new Set(["850", "855", "860", "870", "950"]);
  const SCHOOL_BUS_CODE_PATTERN = /^(55|59)\d\d$/;

  // Versjonsstempel (så du ser at riktig JS er lastet)
  ...
  if (qs.get("controls") === "0") {
    const el = root.querySelector('[data-livekart-role="layerControls"]');
  ...
  function setStatus({ level, text, meta }) {
    ...
    if (metaTextEl) metaTextEl.textContent = meta || "—";
    if (dotEl) {
  ...
  function renderVehicles(json) {
    ...
    for (const v of vehicles) {
      ...
      candidates.push({ v, code: String(code), boat, t: lastUpdatedMs(v.lastUpdated) });
    }

    // 2) Ghost-cap: hvis rute 870 får >2 aktive, behold bare de 2 nyeste
    const final = [];
    let hiddenGhost = 0;

    const grouped = new Map(); // code -> items
    for (const item of candidates) {
      if (item.boat && GHOST_CAP_CODES.has(item.code)) {
    ...
    for (const [code, items] of grouped.entries()) {
      ...
      const keep = items.slice(0, MAX_ACTIVE_BOATS_PER_ROUTE);
      hiddenGhost += Math.max(0, items.length - keep.length);
      final.push(...keep);
    }
